// ================================================================
//  GESTÃO DO TERREIRO — Ile Ase Vodun Ogum Ayres — Apps Script v9
//  - Login por e-mail
//  - Senha como SHA-256 + salt (nunca texto puro na planilha)
//  - Aba Admins criada VAZIA — primeiro acesso cria o Pai de Santo
//  - 3 tentativas → bloqueio de 30 minutos
//  - Desbloqueio pelo app (não pela planilha)
//  - Acesso dev oculto via devkey na URL
//  - Token de sessão 8h no PropertiesService
//  - Planilha é DB puro — nenhuma ação de gestão exige editar planilha
// ================================================================
...
const DEV_KEY = 'ile_ase_dev_2024_falsp'; // troque antes de ir para produção
...
// ── ABA ADMINS (criada VAZIA) ────────────────────────────
function _criarAdmins(ss) {
  const aba = ss.insertSheet(ABA_ADMINS);
  // Colunas: E-mail | Senha | Nome | Nível | Permissões | Tentativas | Bloqueado até | Último acesso
  _cabecalho(aba,
  ...
  aba.setFrozenRows(1);
  // Nenhum usuário padrão — primeiro acesso pelo admin.html cria o Pai de Santo
}
...
function _adminsVazio() {
  ...
  if (!aba) return true;
  return aba.getLastRow() <= 1; // só cabeçalho ou vazia
}
...
function _autenticar(email, senha) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (bloqAte) {
      try {
        if (new Date(bloqAte) > new Date())
          return { ok: false, erro: 'Acesso bloqueado temporariamente. Aguarde 30 minutos ou solicite desbloqueio ao Pai de Santo.' };
      } catch(e) {}
    }
    ...
    if (_hash(senha) !== rows[i][1]) {
      ...
      if (tentativas >= 3) {
        const bloq = new Date();
        bloq.setMinutes(bloq.getMinutes() + 30);
    ...
    // Sucesso — zera tentativas, registra acesso
    aba.getRange(i+1, 6).setValue(0);
    ...
    return {
      ok: true,
      token: _gerarToken(email, permissoes),
      nome:  rows[i][2],
      nivel: rows[i][3],
      permissoes
    };
...
// ── PRIMEIRO ACESSO (cria Pai de Santo) ──────────────────
function _criarPrimeirAdmin(email, senha, nome) {
  if (!_adminsVazio())
    return { ok: false, erro: 'Já existe um administrador cadastrado.' };
...
function _gerarToken(email, permissoes) {
  ...
  PropertiesService.getScriptProperties().setProperty(
    'tok_' + token,
    JSON.stringify({ email, permissoes, expira: expira.toISOString() })
  );
  return token;
}
...
function doPost(e) {
  try {
    ...
    if (d.acao === 'admin-criar') {
      if (!tem('configuracoes')) return saida({ ok: false, erro: 'Sem permissão.', code: 403 });
      return _criarAdmin(d);
    }
    if (d.acao === 'admin-desbloquear') {
      if (!tem('configuracoes')) return saida({ ok: false, erro: 'Sem permissão.', code: 403 });
      return _desbloquearAdmin(d.email);
    }
...
function doGet(e) {
  try {
    ...
    if (acao === 'financeiro-resumo') {
      if (!tem('financeiro')) return saida({ ok: false, erro: 'Sem permissão.', code: 403 });
      return _resumoFinanceiro();
    }
    if (acao === 'admins-listar') {
      if (!tem('configuracoes')) return saida({ ok: false, erro: 'Sem permissão.', code: 403 });
      return _listarAdmins();
    }

    return saida({ ok: true, msg: 'Gestão do Terreiro — Ile Ase Vodun Ogum Ayres — v9' });
  } catch(err) { return saida({ ok: false, erro: err.message }); }
}
...
function _criarAdmin(d) {
  ...
  const rows = aba.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    if ((rows[i][0]||'').toLowerCase() === d.email.toLowerCase())
      return saida({ ok: false, erro: 'E-mail já cadastrado.' });
  }
  aba.appendRow([
    d.email.toLowerCase().trim(), _hash(d.senha),
    d.nome.trim(), d.nivel || 'Admin',
    (d.permissoes || []).join(','),
    0, '', _dataFormatada()
  ]);
...
function _listarAdmins() {
  ...
  return saida({ ok: true, admins: rows.filter(l => l[0] !== '').map(l => ({
    ...
    bloqAte:      l[6] ? _dataFormatada_(l[6]) : '',
    ultimoAcesso: l[7]
  }))});
}
...
function _resumoFinanceiro() {
  ...
  return saida({ok:true,saldo:tE-tS,totalEntradas:tE,totalSaidas:tS,
    entradasMes:eM,saidasMes:sM,saldoMes:eM-sM,porCategoria:pc});
}
...
function _cabecalho(aba,cols,bg,fg){
  aba.getRange(1,1,1,cols.length).setValues([cols]).setBackground(bg).setFontColor(fg)
    .setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center')
    .setVerticalAlignment('middle').setWrap(false);
  aba.setRowHeight(1,34);aba.setFrozenRows(1);
}
function _validacao(aba,cel,lista){aba.getRange(cel).setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(lista,true).setAllowInvalid(false).build());}
function _formatCond(aba,range,regras){aba.setConditionalFormatRules(regras.map(({v,bg,f})=>SpreadsheetApp.newConditionalFormatRule().whenTextEqualTo(v).setBackground(bg).setFontColor(f).setRanges([aba.getRange(range)]).build()));}
function _novaAba(ss,nome){return ss.insertSheet(nome);}
function _excluirPadrao(ss){['Página1','Planilha1','Plan1','Sheet1'].forEach(n=>{const s=ss.getSheetByName(n);if(s)try{ss.deleteSheet(s);}catch(e){}});}
...
function _criarConsumiveis(ss){
  ...
  .forEach(([cat,nome,unid,atual,min],idx)=>{const{label,pct}=_nivelEstoque(atual,min);aba.getRange(idx+2,1,1,14).setValues([['CSM-'+String(idx+1).padStart(3,'0'),cat,nome,unid,atual,min,pct+'%',label,'','',1,'','',dt]]);});
  aba.getRange(1,1,2,14).createFilter();
}
...
function _criarEntidades(ss){
  ...
  _cabecalho(aba,['Orixá / Entidade','Nação / Qualidade','Data da Festa','Dia da Semana','Cores','Oferendas Preferidas','Bebidas / Alimentos','Itens do Acervo','Saudação','Observações Rituais'],'#1a2a1a','#c8f0c8');
  aba.getRange('C2:C50').setNumberFormat('dd/MM');
  const dados=[['Exu','Todas','','Segunda','Preto e vermelho','Pimenta, dendê, farofa, cachaça','Cachaça, vinho tinto','Ogó, tridentes, sete chaves','Laroyê Exu!','Guardião das encruzilhadas'],['Ogum','Ketu / Angola','23/04','Terça','Verde e preto','Feijão preto, carne, dendê','Cerveja preta, vinho tinto','Espada, ferramentas de ferro','Ogum Yê!','Orixá do ferro e da guerra'],['Oxossi','Ketu','','Quinta','Azul e verde','Milho branco, inhame, mel','Mel, água de coco','Arco e flecha','Okê Arô!','Caçador, orixá da fartura'],['Xangô','Ketu','04/12','Quarta','Vermelho e branco','Acarajé, vatapá, azeite','Vinho tinto, cerveja','Machado duplo (oxé)','Kaô Kabiesilê!','Senhor da justiça'],['Oxum','Ketu','08/12','Sábado','Amarelo e dourado','Mel, acarajé, milho amarelo','Mel, champanhe, laranja','Abebê, espelho, leque','Ora Iê Iê Ô!','Orixá do amor e das águas doces'],['Iemanjá','Ketu','02/02','Sábado','Azul e branco','Melão, uva branca, arroz','Champanhe, água, leite','Abebê de prata','Odoyá!','Rainha do mar'],['Oxalá','Ketu','','Sexta','Branco','Inhame, arroz, canjica branca','Água, leite','Opaxorô, pano da costa branco','Êpa Babá!','Pai criador, orixá da paz'],['Iansã','Ketu','04/12','Terça','Vermelho e marrom','Acarajé, abará','Vinho tinto, cerveja','Espada, eruexim','Eparrêi Iansã!','Orixá dos ventos e dos raios'],['Nanã','Ketu','26/07','Segunda','Roxo e branco','Inhame, milho, canjica','Água, leite','Ibiri (vassoura de palha)','Salúbà Nanã!','Mais antiga dos orixás, orixá da lama'],['Obaluaê / Omolu','Ketu','','Segunda','Preto, branco e vermelho','Pipoca, milho, coco','Vinho tinto, dendê','Xaxará','Atotô Obaluaê!','Orixá da saúde e das doenças'],['Preto-Velho','Umbanda','13/05','Segunda','Branco e preto','Fumo, cachaça, mel','Cachaça, mel','Cachimbo, bengala','Ave, meu filho!','Espírito de sabedoria e cura'],['Caboclo','Umbanda','','Terça','Verde e amarelo','Mel, frutas, charuto','Cerveja, cachaça','Arco e flecha, cocar','Okê Caboclo!','Espírito da natureza'],['Exu / Pombagira','Umbanda','','Segunda e sexta','Vermelho e preto','Rosa vermelha, champanhe','Champanhe, vinho tinto','Rosas, tridentes','Laroyê!','Guardião(ã) das encruzilhadas']];
  aba.getRange(2,1,dados.length,10).setValues(dados);
  aba.getRange(1,1,2,10).createFilter();
}
function _criarFinanceiro(ss){
  const aba=_novaAba(ss,ABA_FINANCEIRO);
  _cabecalho(aba,['ID','Data','Tipo','Categoria','Descrição','Valor (R$)','Responsável','Comprovante','Observações'],'#1a2a0a','#d0f0b0');
...
function recalcularEstoque(){
  ...
  for(let i=1;i<rows.length;i++){const a=Number(rows[i][4])||0,m=Number(rows[i][5])||0;if(m>0){const{label,pct}=_nivelEstoque(a,m);aba.getRange(i+1,7).setValue(pct+'%');aba.getRange(i+1,8).setValue(label);n++;}}
...
function gerarListaCompras(){
  ...
  ['Urgente','Alerta','Repor'].forEach(t=>{const g=nok.filter(r=>String(r[7]).includes(t));if(g.length){msg+='\n'+(t==='Urgente'?'🚨':t==='Alerta'?'🔴':'⚠️')+' '+t+':\n';g.forEach(r=>msg+='  • '+r[2]+' — faltam '+Math.max(0,r[5]-r[4])+' '+r[3]+'\n');}});
...
function verDatasMes(){
  ...
  const aF=ss.getSheetByName(ABA_FILHOS);
  if(aF&&aF.getLastRow()>1){const aniv=aF.getDataRange().getValues().slice(1).filter(r=>{try{return r[4]&&new Date(r[4]).getMonth()===mes;}catch(e){return false;}});if(aniv.length){msg+='\n🎂 Aniversariantes:\n';aniv.forEach(r=>msg+='  • '+_dataFormatada_(r[4])+' — '+(r[1]||r[2])+'\n');}}
  const aE=ss.getSheetByName(ABA_ENTIDADES);
  if(aE&&aE.getLastRow()>1){const f=aE.getDataRange().getValues().slice(1).filter(r=>{if(!r[2]||r[2]==='')return false;const p=String(r[2]).split('/');return p.length>=2&&(parseInt(p[1])-1)===mes;});if(f.length){msg+='\n🥁 Festas:\n';f.forEach(r=>msg+='  • '+r[2]+' — '+r[0]+'\n');}}
  SpreadsheetApp.getUi().alert(msg);
}
function verObrigacoes(){
  ...
  const prox=aba.getDataRange().getValues().slice(1).filter(r=>{if(!r[9])return false;try{const d=new Date(r[9]);return d>=hoje&&d<=lim;}catch(e){return false;}});
...
// ── HELPERS ──────────────────────────────────────────────
function _nivelEstoque(a,m){if(!m)return{nivel:'sem-minimo',label:'— Sem mínimo',pct:0};const pct=Math.round((a/m)*100);if(pct>=75)return{nivel:'ok',label:'✅ OK',pct};if(pct>=25)return{nivel:'repor',label:'⚠️ Repor',pct};if(pct>=5)return{nivel:'alerta',label:'🔴 Alerta',pct};return{nivel:'urgente',label:'🚨 Urgente / Zero',pct};}
